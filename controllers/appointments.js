...
exports.getAppointments = async (req, res, next) => {
  let query;
  // General user can only see their appointments
...
exports.addAppointment = async (req, res, next) => {
  try {
    ...
    if (req.user.role !== "admin" && existingAppointment.length >= 3) {
...
exports.deleteAppointment = async (req, res, next) => {
  try {
    ...
    if (!appointment) {
      return res.status(404).json({
        success: false,
        message: `Cannot find appointment with id of ${req.params.id}`,
      });
    }
